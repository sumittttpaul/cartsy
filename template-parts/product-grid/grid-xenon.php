...
<div <?php wc_product_class('cartsy-grid-xenon', $product); ?>>

    <?php
    $cartItems = $splicedGalleryIDs = $pickedProductCategory = [];
    $pickedProductCategoryName = $pickedProductCategoryID = '';
    ...
    $averageRating = get_post_meta($product->get_id(), '_wc_average_rating', true);

    $exclude_cat_ids   = array();
    $exclude_cat_names = array("Uncategorized");
    if (!empty($exclude_cat_names)) {
        foreach ($exclude_cat_names as $name) {
            if (!empty($name)) {
                $excluded_term = get_term_by('name', $name, 'product_cat');
                if (!empty($excluded_term)) {
                    $exclude_cat_ids[] = (int) $excluded_term->term_id;
                }
            }
        }
    }
    ...
    $pickedProductCategory = get_categories($args);

    if (!empty($pickedProductCategory)) {
        $pickedProductCategoryName = $pickedProductCategory[0]->name;
        $pickedProductCategoryID = $pickedProductCategory[0]->term_id;
    }
    ?>
    ...
    <div class="cartsy-xenon-product-card">
        <div class="cartsy-xenon-product-card-thumb">
            <?php if ($isOnSale) { ?>
        ...
        <div class=" cartsy-xenon-product-card-description">
            <div class="cartsy-xenon-product-card-info">
                <a href="<?php echo esc_url(get_term_link($pickedProductCategoryID, 'product_cat')); ?>" class="cartsy-xenon-product-card-category">
                    <?php echo esc_attr($pickedProductCategoryName); ?>
                </a>
                <a href="<?php echo esc_url($link); ?>" class="cartsy-xenon-product-card-title">
                    <?php woocommerce_template_loop_product_title(); ?>
                </a>
                <div class="cartsy-rating-star">

                    <?php

                    for ($i = 0; $i < 5; $i++) { ?>
                        <svg xmlns="http://www.w3.org/2000/svg" class='rating-star' width="12.466" height="12" viewBox="0 0 12.466 12">
                            <path d="M12.466,14.152l-4.53-.3L6.23,9.569,4.525,13.855,0,14.152,3.47,17.1l-1.139,4.47L6.23,19.1l3.9,2.464L8.991,17.1Z" transform="translate(0 -9.569)" fill="currentColor" />
                        </svg>
                    <?php } ?>
                    ...
                    <span>
                        <?php if ($averageRating !== 0) { ?>
                            <?php for ($i = 0; $i < 5; $i++) { ?>
                                <?php if (!empty($averageRating) && ($averageRating - $i >= 1)) { ?>
                                    <svg xmlns="http://www.w3.org/2000/svg" class='rating-star' width="12.466" height="12" viewBox="0 0 12.466 12">
                                        <path d="M12.466,14.152l-4.53-.3L6.23,9.569,4.525,13.855,0,14.152,3.47,17.1l-1.139,4.47L6.23,19.1l3.9,2.464L8.991,17.1Z" transform="translate(0 -9.569)" fill="currentColor" />
                                    </svg>
                                <?php } elseif (!empty($averageRating) && ($averageRating - $i < 1) && ($averageRating - $i > 0)) { ?>
                                    <svg xmlns="http://www.w3.org/2000/svg" class='rating-star' width="6.5" height="12" viewBox="0 0 6.5 12">
                                        <path d="M12.466,14.152l-4.53-.3L6.23,9.569,4.525,13.855,0,14.152,3.47,17.1l-1.139,4.47L6.23,19.1l3.9,2.464L8.991,17.1Z" transform="translate(0 -9.569)" fill="currentColor" />
                                    </svg>
                                <?php } ?>
                            <?php } ?>
                ...
                <div class="cartsy-xenon-product-card-price">
                    <span class="price">
            ...
            <?php if (class_exists('CartsyHelper')) { ?>
                <?php if ($isInStock) { ?>
                    <?php if ($productType === 'simple') { ?>
                        <div class="cartsy-xenon-product-card-cart">
                            <!-- Add to cart button -->
                            <a href="#" class="cartsy_btn_display product_type_simple add_to_cart_button cartsy_ajax_add_to_cart cartsy-update-qty cartsy-add-to-cart-<?php echo esc_attr($product->get_id()); ?>" data-product_id="<?php echo esc_attr($product->get_id()); ?>" style="<?php echo esc_attr($cartButton); ?>">
                                <span class="cartsy-xenon-product-card-cart-button">
                                    <span class="label"><?php echo esc_html__('ADD TO CART', 'cartsy'); ?></span>
                                </span>
                            </a>
                            <!-- End -->
                            <!-- Quantity button -->
