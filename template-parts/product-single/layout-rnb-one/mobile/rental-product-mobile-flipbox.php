		...
		<div class="price-showing">
			<div class="<?php echo esc_attr($flip_box[1]); ?>">
				<div class="item-pricing">
					<h5> <?php echo esc_html__('Day based pricing : ', 'cartsy'); ?><?php echo get_the_title($inventory); ?></h5>
					<?php if ($pricing_data['pricing_type'] === 'general_pricing') : ?>
						...
						<div class="rnb-pricing-wrap">
							<div class="day-ranges-pricing-plan">
								<span class="range-days"> <?php echo wc_price($general_price); ?><?php _e(' / days', 'cartsy'); ?> </span>
							</div>
						</div>
					<?php endif; ?>
						...
						<div class="rnb-pricing-wrap">
							<?php if (is_array($pricing_plans) && !empty($pricing_plans)) { ?>
								<?php foreach ($pricing_plans as $key => $value) { ?>
									<?php $rate = $value['cost_applicable'] === 'fixed' ? esc_html__('Fixed', 'cartsy') : esc_html__('/ Day', 'cartsy'); ?>
									<div class="day-ranges-pricing-plan">
										<span class="range-days"><?php echo esc_attr($value['min_days']); ?> - <?php echo esc_attr($value['max_days']); ?> <?php _e('days :', 'cartsy'); ?> </span>
					...
					if ($pricing_data['hourly_pricing_type'] === 'hourly_range') :
						...
						if (is_array($pricing_plans) && !empty($pricing_plans)) : ?>
							<h5><?php echo esc_html__('Hourly based pricing', 'cartsy'); ?></h5>
							<div class="rnb-pricing-wrap">
								<?php foreach ($pricing_plans as $key => $value) : ?>
									<?php $rate = $value['cost_applicable'] === 'fixed' ? esc_html__('Fixed', 'cartsy') : esc_html__('/ Hour', 'cartsy'); ?>
									<div class="day-ranges-pricing-plan">
										<span class="range-days"><?php echo esc_attr($value['min_hours']); ?> - <?php echo esc_attr($value['max_hours']); ?> <?php _e('Hours :', 'cartsy'); ?> </span>
										<span class="range-price"><strong><?php echo wc_price($value['range_cost']); ?></strong> <?php echo esc_attr($rate); ?></span>
									</div>
								<?php endforeach; ?>
							</div>
					...
					<?php if ($pricing_data['hourly_pricing_type'] === 'hourly_general') : ?>
						...
						<?php if (!empty($general_hourly_price)) : ?>
							...
							<p class="hourly-general"><?php echo wc_price($general_hourly_price); ?>
								/ <?php echo esc_html__('per hour', 'cartsy'); ?>
							</p>
						<?php endif; ?>
					<?php endif; ?>
				</div>
